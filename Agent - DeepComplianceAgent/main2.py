...
async def read_and_extract(state: GraphState) -> GraphState:
    filepath = os.getenv("INPUT_FILE", "BASEL 3 requirements.docx")
...
async def reason_about_compliance(state: GraphState) -> GraphState:
    ...
    human_prompt = (
        ...
        "If no issues are found, return an empty JSON array: []. Provide only valid JSON as the response."
    )
    ...
    try:
        ...
        try:
            ...
            logger.exception("Failed to parse LLM findings JSON; storing raw response in findings.")
            fallback = [{
                "issue": "LLM-parsing-error",
                "severity": "Medium",
                "explanation": "Could not parse LLM JSON output. See raw_output in evidence_refs.",
                "confidence": 0.0,
                "evidence_refs": [content[:1000]]
            }]
...
async def act_or_escalate(state: GraphState) -> GraphState:
    ...
    try:
        if action == "auto_resolve":
            ...
            task_id = None
        elif action in ("log_and_monitor",):
            ...
            try:
                ...
                if hasattr(sdk, "actions") and hasattr(sdk.actions, "create"):
                    task_obj = sdk.actions.create(**create_args)
                    task_id = getattr(task_obj, "id", None) or (task_obj.get("id") if isinstance(task_obj, dict) else None)
                elif hasattr(sdk, "actions") and hasattr(sdk.actions, "create_task"):
                    task_obj = sdk.actions.create_task(**create_args)
                    task_id = getattr(task_obj, "id", None) or (task_obj.get("id") if isinstance(task_obj, dict) else None)
                else:
                    logger.warning("UiPath SDK does not expose actions.create - replace with your Action Center creation call.")
...
async def explain_and_record(state: GraphState) -> GraphState:
    ...
    try:
        ...
        bucket_name = os.getenv("AUDIT_BUCKET", "DeepComplianceAgent-Audits")
...
async def learn_from_feedback(state: GraphState) -> GraphState:
    ...
    try:
        ...
        bucket_name = os.getenv("FEEDBACK_BUCKET", "DeepComplianceAgent-Feedback")
